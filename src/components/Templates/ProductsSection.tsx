...
const ProductsSection = ({
  ...
  const onOpenSpoiler = (itemTitle: string) => {
    if (!openedItems.includes(itemTitle)) {
      setOpenedItems((current) => [...current, itemTitle]);
    }
  };
  return (
            ...
            <div className="mx-auto mt-8 max-w-6xl space-y-6 lg:mt-16 lg:space-y-8">
              {product.showcase_items?.map((item) => (
                <Accordion
                  key={item._uid}
                  title={item.title}
                  onToggle={() => onOpenSpoiler(item.title)}
                  content={
                    <>
                      <video
                        width="100%"
                        key={openedItems.includes(item.title).toString()}
                        autoPlay={openedItems.includes(item.title)}
                        muted
                        loop
                          ...
                          <Link href={item.link[0].href}>
                            ...
                            >
                              ...
                              {item.link[0].target === '_blank' && (
                                <i
                                  className={'fa-solid fa-external-link ml-1'}
                                />
                              )}
                            </a>
                          </Link>
